...
and no `async def` in it anywhere -- so this file is a **ratchet on
something already true** rather than a cleanup with work behind it.
That is exactly why it is worth having: the purity is currently kept by
habit, and habit is what erodes one convenience at a time once a
refactor starts moving flow code across the line. See
...
MODEL_ROOT = PROJECT_ROOT / "d12ball"
...
class ModelPurityTests(unittest.TestCase):
    """`d12ball/` is the model, and these are the two rules it keeps."""

    def test_every_model_module_imports_without_discord(self) -> None:
        """
        A fresh interpreter can import all of `d12ball/` with `discord`
        refused outright.
        ...
        probe = subprocess.run(
            [sys.executable, "-c", IMPORT_PROBE, str(MODEL_ROOT)],
            capture_output=True,
            text=True,
        ...
        self.assertEqual(
            failures,
            {},
            "these modules under d12ball/ do not import cleanly without "
            f"discord: {failures}",
        )
