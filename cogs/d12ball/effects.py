...
class ManeuverEffectsMixin:
    ...
    async def apply_dribble_advance(
        ...
        """
        The Discord half of a won Dribble Advance: run the step, save
        what it did, then dispatch what it handed back.

        **The persist is not optional, and it is before the
        dispatch** -- the transition rule through Phase 5, and the
        same line `apply_low_pass` carries. `dribble_advance_step` no
        longer saves itself, and the speed choice underneath is still
        the cog's and ends in a prompt, which hands the turn to a
        click that reloads the match out of the save file. See
        principle 9 in CLAUDE.md.

        Here the save is a *moved* line rather than an added one: the
        old body persisted in the middle of itself, between the move
        and the message.
        """
    ...
    async def apply_dribble_burst(
        ...
        """
        The Discord half of a won Dribble Burst -- the same four lines
        as `apply_dribble_advance`, and the persist matters more here:
        the step charges exhaustion and re-tests the Exhausted
        threshold, and a flag set but never written out is the bug
        `RulesEngine.apply_exhaustion` exists to prevent.
        """
