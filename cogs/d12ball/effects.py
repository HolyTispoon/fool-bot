...
class ManeuverEffectsMixin:
    ...
    async def apply_deflection(
        ...
        Four lines over `deflect_step`, which is where the knock-back,
        the Fullback's extra space, the wording, the overshoot into a
        scoring opportunity and a beaten Setup Pass's cost live -- see
        `apply_low_pass` for the shape and principle 9 in CLAUDE.md for
        why the save is here rather than inside the step.

        **Two saves became this one.** `knock_ball_back` persisted the
        moment the ball moved and the overshoot branch persisted again
        after the turnover; nothing between them could fail, so both
        wrote the same state and the collapse costs nothing -- rank
        D3's finding rather than rank O2's lost tokens.
        """
        result = deflect_step(self.engine, match, key)
        self.persist(game, match)
        await self.dispatch_step_result(interaction, game, match, result)
