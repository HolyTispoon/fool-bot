...
of them. Phase 3 is one template run six times, and **3a, 3b, 3c, 3d and
3e have landed (PR #229, PR #232, PR #233, PR #235, PR #PENDING)**; the
...
| 3e | **Landed (PR #PENDING).** `Rank D1 -- Deflect and Clear.` It was the rank `board_changed` had been deferred to: the answer is that a step reports the position and `D12Ball.follow_on_posts_its_own_board` declines the write where the follow-on draws the board itself, which is the answer for all eight `begin_loose_ball` callers. Two members, `BEGIN_LOOSE_BALL` and `OFFER_SETUP_PASS_PUSH_BACK`. `resolve_low_pass`'s no-teammate branch stayed where it is, because PR #232's two rules questions on it are still unanswered. |
| 3f | `Rank O3 -- High Pass and Setup Pass. The hardest by a distance: the overshoot, the contest, out-of-bounds into begin_ball_recovery. throw_high_pass saves itself today -- strip it and the wrapper persists; it is the last of the five self-saving functions. Read docs/design/shooting.md and docs/design/loose-balls.md (the High Pass exemption). Four things 3e settled for you, so do not reopen them: board_changed reports the position and nothing else, and the frontend declines the duplicate write through D12Ball.follow_on_posts_its_own_board -- whose High Pass exemption is already written, so a hand-off carrying is_high_pass=True still gets its refresh. BEGIN_LOOSE_BALL and OFFER_SETUP_PASS_PUSH_BACK already exist and already have rows, so O3 adds a member only for begin_ball_recovery and for anything else it is first to name. Setup Pass's cost is not yours: rank D1's step dispatches the push-back, and neither offer_setup_pass_push_back nor apply_setup_pass_push_back moved -- what is yours is the card being beaten. And begin_loose_ball's distance_moved is already read by name rather than off await_args.args in the two assertions that had it positional. offer_speed_choice's last direct caller in cogs/ is still resolve_setup_pass, which is yours.` |
