...
Only maneuvers change it. In basic mode there are four: Low Pass adds 1, Deflect takes 1 off, and Dribble Advance and Steal let the player change it by up to their own skill. The [gambits](#gambits) on their ranks do the same in larger amounts — Skilled Pass adds 3, Clear takes 3 off, Setup Pass and Intercept let the player change it by up to their own skill, and Dribble Burst lets the handler set it to anything from 1 to 12.
