...
in `d12ball/tts.py`, the way a card layout is. Two reasons over a
third-party mesh: the item is public, and a mesh off Thingiverse is a
licence to check and credit on a Workshop page; and a generated meeple's
shape is data like everything else on the table, so the next person who
wants it a little fatter edits a polygon rather than a binary. The same
mesh serves as its own collider.
