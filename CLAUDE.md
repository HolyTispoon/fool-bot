...
| `d12ball/box_art.py` | The five printed things around the game rather than in it: the box cover, one side (the box is square), the underside, the one-page sale sheet, and the playtest card whose back carries the survey QR -- all on white, because a dark cover is a press run nobody wants to pay for -- plus a night cover and banners (a wide one and the Screentop table's 1280x720) for screens. Every claim on them is read from the game and every quoted rule is the books' own, word for word; the picture of the game is the printed board with the Screentop table's own meeples on it, and the ball is a d12 drawn as a solid made of something. `scripts/render_box_art.py` is its CLI -- [box-and-sale-sheet.md](docs/design/box-and-sale-sheet.md) |
