...
python3 foolbot.py
FOOLBOT_WEB_PORT=8080 python3 foolbot.py   # ...and the web app with it
```
...
| `d12ball/game.py` | `D12BallGame` (the game record -- its three Discord ids optional, since a web game has none -- with every rule about the lobby and setup as a method that refuses with `RuleRefusal`: `lobby_join`/`observe`/`leave`, `configure` over `GAME_SETTINGS`, `start_lobby`, `pick_team` and `excluded_teams`, the coin and the sides), `RuleRefusal`, `ADVANCED_MODULES`, `Team`, `TEAM_PAIRS`, `GameMode`, `Formation` |
...
| `webapp/` | **The web frontend** over the same `GameService` the cog uses -- `server.py` (the aiohttp app, the routes, the journal), `present.py` (the tokens as the bot's own emoji, and one control builder per `PromptKind`, off `PendingPrompt.options` and nothing else, each control in Discord's colour for it), `board.py` (the position as a layout the page draws in the bot's board layout, every value the PNG asks the rules for asked of the same function), `pictures.py` (the model's own card faces, maneuver cards and emoji, served), `keys.py` (a key per coach per game, derived and never stored) and `static/` (one page: the board and the prompt, beside the jumbotron, the game log and the chat, in Discord's colours). The chat is people talking, in memory, and goes nowhere near the service. It imports neither `discord` nor `cogs`; `FOOLBOT_WEB_PORT` is the switch -- [web-app.md](docs/design/web-app.md) |
| `gamelocks.py` | One `asyncio.Lock` per game, shared by both frontends: held around a click's whole answer, the apply *and* what it puts up -- [web-app.md](docs/design/web-app.md) |
...
| `webapp/`, `gamelocks.py`, `d12ball/wire.py`, `GameService.listeners`, `SafeView._scheduled_task`, `RulesEngine.cyborg_condition_ids`, `/d12ball web_link`, `tests/test_web_app.py`, `tests/test_web_board.py`, `tests/test_web_purity.py`, `tests/test_game_locks.py`, `tests/test_wire_shapes.py`, `FOOLBOT_WEB_*` | [web-app.md](docs/design/web-app.md) | What a second frontend may not do and how it is measured; the wire shapes and why they are one-way; who is on the other end of a request and what is not sent to them; one process, one service and what the lock is actually for; why a page sees a turn taken on Discord; the page's two columns, and why the board is HTML in the bot's layout with the model's own cards on it; the chat, and why it is not the model's voice |
