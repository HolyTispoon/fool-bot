...
- **Suppress saves through `tests/save_patches.py`** (`suppressed_cog_saves`, which reaches the service's save; `suppressed_view_saves` patches nothing since no view saves, and is kept for the call sites that name both). A patch on a package name reaches none of its submodules. A full run must not create `data/`.
