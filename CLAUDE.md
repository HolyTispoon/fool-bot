...
| `d12ball/box_art.py` | The five printed things around the game rather than in it: the box cover, one side (the box is square), the underside, the one-page sale sheet, and the playtest card whose back carries the survey QR -- all on white, because a dark cover is a press run nobody wants to pay for -- plus a night cover and a wide banner for screens. Every claim on them is read from the game and every quoted rule is the books' own, word for word; the picture of the game is the printed board with the Screentop table's own meeples on it, and the ball is a d12 drawn as a solid made of something. `scripts/render_box_art.py` is its CLI -- [box-and-sale-sheet.md](docs/design/box-and-sale-sheet.md) |
...
| `webapp/` | **The web frontend** over the same `GameService` the cog uses -- `server.py` (the aiohttp app, the routes, the journal and the board it draws), `present.py` (the tokens, and one control builder per `PromptKind`, off `PendingPrompt.options` and nothing else), `keys.py` (a key per coach per game, derived and never stored) and `static/` (one page). It imports neither `discord` nor `cogs`; `FOOLBOT_WEB_PORT` is the switch -- [web-app.md](docs/design/web-app.md) |
...
| `d12ball/box_art.py`, `scripts/render_box_art.py`, the box panels, the sale sheet, the playtest card and its QR | [box-and-sale-sheet.md](docs/design/box-and-sale-sheet.md) | Why everything printed is white and what the one night cover is for; the box cut from the folded field board; why the box may not word a rule for itself; why no playing time and no age are printed; the printed board as the picture, the meeple read from the table's own path and the d12 that is a solid made of something; what a banner is and why it is not a cropped cover; the cover's four and why their names are written down; the QR drawn module by module and its size floor |
...
| `webapp/`, `gamelocks.py`, `d12ball/wire.py`, `GameService.listeners`, `SafeView._scheduled_task`, `RulesEngine.cyborg_condition_ids`, `/d12ball web_link`, `tests/test_web_app.py`, `tests/test_web_purity.py`, `tests/test_game_locks.py`, `tests/test_wire_shapes.py`, `FOOLBOT_WEB_*` | [web-app.md](docs/design/web-app.md) | What a second frontend may not do and how it is measured; the wire shapes and why they are one-way; who is on the other end of a request and what is not sent to them; one process, one service and what the lock is actually for; why a page sees a turn taken on Discord |
