...
| `docs/web-app-redesign.md` | The worksheet the web app redesign is being built from, off the design prototype the author reviewed on 2026-09-26 (the canvas it links): what the prototype settled -- Discord's chrome kept and the play area drawn from scratch, no coloured buttons because the answer is the thing on the board lit in gold, the Screentop meeple with the species icon, the fan rules for a crowded space, the badges and the ball's corners, the goals, outcomes large and first, the shot brief worded from the Law, one jumbotron bar, the bench boxes, the hover card, four sidebar tabs with the Charter in the page, the phone keeping the field horizontal -- twelve steps in the order they build on each other with a prompt per step and a shared preamble, claimed with `scripts/claim_web_step.py --series redesign <n>` and struck as they land. Supersedes steps 9 to 11 of `docs/web-app-next.md`. **Nothing in it is a rule** |
...
[docs/web-app-next.md](docs/web-app-next.md) was what was built next, and
[docs/web-app-redesign.md](docs/web-app-redesign.md) is what is built now.
