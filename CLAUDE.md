...
| `d12ball/game.py` | `D12BallGame` (the game record -- its three Discord ids optional, since a web game has none -- with every rule about the lobby and setup as a method that refuses with `RuleRefusal`: `lobby_join`/`observe`/`leave`, a web room's seat moves (`take_seat`, `vacate_seat`, `seat_ai`, `unseat_ai`, over `ai_seats` and its one reading `ai_holds`), `configure` over `GAME_SETTINGS`, `start_lobby`, `pick_team` and `excluded_teams`, the coin and the sides), `RuleRefusal`, `Team`, `TEAM_PAIRS`, `GameMode` (training, basic, advanced), `Formation` |
...
| `d12ball/species_cards.py` | The four species abilities as a three-card reference set: printed, and posted by `/d12ball species_abilities_reference` |
| `d12ball/role_cards.py` | The six basic role abilities as a one-card reference set: printed, and posted by `/d12ball role_abilities_reference` |
...
| `gamesaves/d12ball/service.py` | **`GameService`**, the one door for a change to a game (ARCHITECTURE.md, part 2): `apply_action`, `begin`, `resume`, `run_step`, each load-apply-save-once-return, and `GameResult`, what a frontend renders. `run` answers for an AI side wherever the loop reaches its question, batched by the frontend's `Batching.carry_answer`. The setup methods that need no turn -- `create_game`, the lobby moves, `configure`, `pick_team`, `flip_coin`, `choose_home_or_visiting` -- are each a thin door over a rule on `D12BallGame` -- [game-service.md](docs/design/game-service.md) |
...
| `webapp/` | **The web frontend, a system of its own**: `python3 -m webapp` runs it as its own process, with no token, over its own games (`data/d12ball_web_games.json`) and its own `GameService`, engine and locks, built from the same model the cog's are (`server.build_service`). It shares the model with the bot and nothing at runtime; no game is on both. `server.py` (the aiohttp app, the routes, the journal, `main`), `present.py` (the tokens as the bot's own emoji, and one control builder per `PromptKind`, off `PendingPrompt.options` and nothing else, each control in Discord's colour for it), `board.py` (the position as a layout the page draws in the bot's board layout, every value the PNG asks the rules for asked of the same function), `pictures.py` (the model's own card faces, maneuver cards and emoji, served), `identity.py` (who is reading: a name and an id in a signed cookie, stored nowhere), `rooms.py` (the web app's own state per room -- admins and who has been in -- in `data/d12ball_web_rooms.json`, never the save), `keys.py` (the one secret the cookie is signed under) and `static/` (one page: the board and the prompt, beside the jumbotron, the game log and the chat, in Discord's colours). The chat is people talking, in memory, and goes nowhere near the service. It imports neither `discord` nor `cogs`, nothing in the bot imports it, and it never names the bot's games file -- [web-app.md](docs/design/web-app.md) |
...
| `docs/web-app-next.md` | The worksheet the web app's next steps are being done from, under the author's decision of 2026-09-25 that **the web app and the bot are two parallel systems sharing the model and nothing at runtime** -- no mixed games, its own process, its own games file. What that changes in the shipped design and what it costs, why it still stays in this repository, the decisions taken in its review (rooms whose first two arrivals are the coaches and the rest observers, a seat that may change hands mid-game and an admin who may kick one, the `K:\` machine, the wire tree kept), ten steps in the order they pay off with a Claude Code prompt per step -- including a chat in the room, and the web games' statistics read on Discord, the one read across the line -- and what is deliberately not on the list. **Nothing in it is a rule** |
