...
| `d12ball/flow/` | The turn's flow with no Discord in it: `StepResult` and the transitional `FollowOn` in `result.py`, **all twelve maneuvers** in `effects.py` (nine steps -- five cards are their rank-mate parameterised, and Setup Pass is three), the out-of-bounds pickup in `turnovers.py` and the injury queue in `rolls.py`. A step changes the match and says what happened; it sends nothing and saves nothing -- [model-discord-split.md](docs/design/model-discord-split.md) |
