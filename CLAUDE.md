...
| `d12ball/flow/` | The turn's flow with no Discord in it: `StepResult` and the transitional `FollowOn` in `result.py`, and the maneuver steps lifted so far in `effects.py` (Low Pass and Skilled Pass; Dribble Advance and Dribble Burst). A step changes the match and says what happened; it sends nothing and saves nothing -- [model-discord-split.md](docs/design/model-discord-split.md) |
