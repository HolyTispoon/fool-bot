...
| `d12ball/box_art.py` | The five printed things around the game rather than in it: the box cover, one side (the box is square), the underside, the one-page sale sheet, and the playtest card whose back carries the survey QR. Every claim on them is read from the game and every quoted rule is the books' own, word for word. `scripts/render_box_art.py` is its CLI -- [box-and-sale-sheet.md](docs/design/box-and-sale-sheet.md) |
...
| `d12ball/box_art.py`, `scripts/render_box_art.py`, the box panels, the sale sheet, the playtest card and its QR | [box-and-sale-sheet.md](docs/design/box-and-sale-sheet.md) | Why these are the bot's night palette and the boards are not; the box cut from the folded field board; why the box may not word a rule for itself; why no playing time and no age are printed; the cover's four and why their names are written down; the QR drawn module by module and its size floor |
