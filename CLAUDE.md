...
| `docs/web-app-next.md` | The worksheet the web app's next steps are being done from -- where it stood on 2026-09-25 (it plays a running game and Discord does not see a web turn), why it stays in this repository, the three decisions the author takes first, eight steps in the order they pay off with a Claude Code prompt per step, and what is deliberately not on the list. **Nothing in it is a rule** |
