...
| `d12ball/flow/` | The turn's flow with no Discord in it: `StepResult` and the transitional `FollowOn` in `result.py`, and **all twelve maneuvers** in `effects.py`, as nine steps (five cards are their rank-mate parameterised, and Setup Pass is three). A step changes the match and says what happened; it sends nothing and saves nothing -- [model-discord-split.md](docs/design/model-discord-split.md) |
