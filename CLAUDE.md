...
| `cogs/d12ball/` | All D12 Ball slash commands and the Discord interaction flow, as six mixins assembled into `D12Ball` in `__init__.py`: `core` (lifecycle, lookups, `persist`, `dispatch_step_result` and the follow-on table, `pending_turn_view`), `effects` (one banner per maneuver, and how a loose ball reaches a channel), `turnovers` (the run back's batching, the time out, the pickup prompts), `periods` (clock, halftime, shootout), `presentation` (prompts, images, channels, the AI turn, tutorial narration), `slash_commands` (every command, the three subgroups, the startup sweep). **Since Phase 4 the spine's decisions are `d12ball/flow/`'s and what is left here is the posting** -- [cog-structure.md](docs/design/cog-structure.md) |
...
| `d12ball/flow/` | The turn's flow with no Discord in it: `StepResult` and the transitional `FollowOn` in `result.py`; **all twelve maneuvers** in `effects.py`, as nine steps (five cards are their rank-mate parameterised, and Setup Pass is three); and since Phase 4 **the spine** -- `arrival.py` (the two gates, the loose ball, the High Pass contest, the set-up offer), `resolution.py` (the tail of a maneuver), `turnovers.py` (the run back and the pickup after it) and `maneuver.py` (challenger to settled maneuver, and the injury queue). A step changes the match and says what happened; it sends nothing and saves nothing -- [model-discord-split.md](docs/design/model-discord-split.md) |
