...
class FollowOnStep(Enum):
    ...
    What the enum records is **where one step ends and the next
    begins**: the joints of a turn, which the loop runs and a frontend
    renders around. A frontend reads the member to decide what becomes
    a message of its own (`NarrationGroup.step`, the `own_message` set
    it hands `advance`), where a picture goes, and where the loop must
    stop for one to be taken (`stop_after`). A step is *named* rather
    than called wherever the boundary is one a frontend has something
    to decide at. A closed set, so nothing on either side can reach a
    step by spelling its name; the values carry nothing and a save
    holds the member's name (`FollowOn.to_dict`).

    The member notes are the *ordering* decisions each joint records --
    what is announced before what, and which line rides inside which
    message -- which are the rules a second frontend has to keep. (How
    the enum grew during the model/Discord split is history, and is in
    ...
    #: anyone is asked at all is decided here -- a speed with one
    #: target is set without asking. A steal owes the same choice and
    #: does **not** name this member: it rides on `BEGIN_RUN_BACK`'s
    #: `speed_choice_after` flag and `finish_run_back` offers it, once
    #: everybody is back.
    ...
    #: is standing on that space decides how it is won. The step
    #: **announces the position with the board under it**, because the
    #: ball is lying somewhere nothing in the channel has named -- a
    #: stop a frontend draws (`Batching.stop_after`). A step naming
    #: this one still reports `board_changed` honestly; a frontend that
    ...
    #: (`finish_maneuver_resolution` and `begin_run_back`) would
    #: otherwise carry their lines into it as one paragraph; it is in
    #: the frontend's `own_message` set for that.
    END_PERIOD = auto()
    #: The offensive choice, handed back to whoever now has the ball --
    #: the last thing an ordinary turn does. Two steps: this one stages
    #: a tutorial beat and holds its lesson behind a Continue
    #: (`d12ball.flow.turn.begin_turn`), and `START_TURN` is what comes
    #: after the click -- the lone handler picked without asking, or
    #: the prompt. Split because a gate's continuation has to be a
    #: step that does not stage the beat a second time (see
    #: `d12ball.flow.gates`).
    SEND_TURN_PROMPT = auto()
    START_TURN = auto()
    #: A coaching window, on any of its five occasions
    #: (`d12ball.flow.windows.open_substitution_window`). Named because
    #: the window's prompt is the one in the game that carries **the
    #: coach's own half-field**, and because the tutorial's coaching
    #: explainer gates the whole of it behind a Continue -- a picture
    #: and a gate. `heading` rides in `kwargs` rather than as
    #: narration and is not a lead-in: it goes *inside* the prompt,
    #: above the allowance, where `RulesEngine.coaching_prompt` puts it.
    BEGIN_SUBSTITUTION_WINDOW = auto()
    #: The score attempt a set-up leads into: the composition image
    #: and the roll prompt, which is two uploads and no decision.
    START_SET_UP_SHOT = auto()
    #: The run-back cascade, one pass at a time
    #: (`d12ball.flow.turnovers.run_back_passes`). Named so a frontend
    #: may batch its automatic placements into one message and one
    #: board refresh, which is a Discord economy and not a rule
    #: (principle 8); an AI side's placement is composed into the same
    #: message (`GameService._answer_for_ai`).
    CONTINUE_RUN_BACK = auto()
    #: The front half of a turn, each a joint a frontend has something
    #: to put up at: `BEGIN_MANEUVER_ACTION_SELECTION` is where a
    #: challenger's pick hands on; `SEND_MANEUVER_ACTION_PROMPT` is the
    #: prompt it ends on, which carries the hand image, the full-size
    #: link, the field strip and a tutorial's Continue gate.
    #: `RESOLVE_MANEUVER` is the reveal. `BEGIN_EFFECT_RESOLUTION`
    #: dispatches a won card to its effect. `BEGIN_MANEUVER_SKILL_TEST`
    #: charges both participants and puts the roll behind a button,
    #: and takes the reveal as its `headline` rather than as narration:
    #: it embeds the line in its own message instead of posting one
    #: above it.
    ...
    #: because the frontend has batched narration in hand that has to
    #: be flushed before it runs -- the ordering is the frontend's, so
    #: the generator names the step and the frontend decides when.
    FINISH_RUN_BACK = auto()
    #: The out-of-bounds pickup, applied -- what `BallRecoveryView`'s
    #: click runs and what the AI's answer runs, so the pickup's
    #: message is the same group whoever answered.
    APPLY_BALL_RECOVERY = auto()
    #: The kickoff board, and the tutorial's welcome over the top of
    #: it: the pin is a stop (`StepResult.new_play`) and the welcome a
    ...
    #: rather than having them posted above it (`Batching.speaks_lines`):
    #: the result and the scoresheet are the content of the message the
    ...
    #: A member of its own rather than `BEGIN_LOOSE_BALL` with a
    #: `headline=` and `is_high_pass=True`, because the two steps do
    #: different things with the board: a High Pass is on a receiver
    #: both coaches watched catch it, so the contest is announced
    #: plainly and the board the pass moved has to be written
    #: **before** it. Folding it into the loose ball's member would
    #: have put it under the loose ball's stop and lost that write.
    BEGIN_HIGH_PASS_CONTEST = auto()
    #: What a settled shootout test hands back to: the next test, or
    #: the end of the shootout (`d12ball.flow.periods.continue_shootout`)
    #: -- a run of separate events, so a row in the frontend's
    #: `own_message` set.
    CONTINUE_SHOOTOUT = auto()
    #: A challenger pick nobody was asked for -- the one defender
    #: already standing on the ball, or the AI's
    #: (`turn.auto_resolve_challenger`). Named because what a frontend
    #: puts up for it is the **challenge image**, the matchup drawn,
    #: and a picture is the frontend's; the group carries the
    #: challenger it named (`Narration.arguments`).
    ...
    #: ahead (`d12ball.flow.windows.finish_substitution_window`). A row
    #: in the frontend's `own_message` set, like the whistle's.
    ...
    #: question for somebody, so a member for it named nothing.) Each
    #: is also called inline by the step that ordinarily reaches it;
    #: the member is the door a resume comes back in through
    #: (`GameService.resume`), so a restart, a resume and a web request
    #: all read the same answer to "is anybody asked here?".
